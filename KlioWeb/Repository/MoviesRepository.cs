...
namespace KlioWeb.Repository
{
    public class MoviesRepository : IMoviesRepository
    {
        ...
        public async Task<IndexYearsDTO> GetIndexYearsDTO()
        {
            var limitLasts = 3;
            double maxViews = (double)context.Movies.Max(p => p.ViewCounter);
            ...
            var lstYears = context.Movies
                .OrderBy(p => p.ReleaseDate)
                .Select(p => p.ReleaseDate.Value.Year)
                .Distinct()
                .AsEnumerable()
            ...
            var response = new IndexYearsDTO();
            response.LastMovie = movieLast;
            response.LastMovieCountries = Countries;
            response.AllYears = lstYears;
            response.LastAdded = moviesLast;
